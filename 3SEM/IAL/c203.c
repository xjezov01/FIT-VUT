...
*/
	if (q != NULL)
	{
		...
		queueError(QERR_INIT); // chyba q == NULL
...
*/
	...
	{
		*c = q->arr[q->f_index]; // načíta do "c" znak zo začiatku fronty
	}
}
...
*/
	...
	{
		q->f_index = nextIndex(q->f_index); // odstranime znak zo začiatku fronty, staci posunut index
	}
}
...
*/
	if (queueEmpty(q))
		queueError(QERR_GET); // fronta je prazdna
	else
	{
		queueFront(q,c); // vratime znak c
		queueRemove(q); // odstránime znak zo začiatku fronty
	}
}
...
*/
	if (queueFull(q))
		queueError(QERR_UP); // fronta je plna
	else
	{
		q->arr[q->b_index] = c;
